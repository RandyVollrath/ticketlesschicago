 ...
 *
 * Pulls the last N hours of parking_diagnostics + parking_location_history,
 * groups by user, classifies each row by failure signature, and writes a
 * structured JSON report to stdout. This script does NOT write any code
 * changes — it produces the data that both the on-demand skill workflow
 * (.claude/skills/parking-quality-improver.md) and the daily email cron
 * (pages/api/cron/parking-quality-daily.ts) consume.
 *
 * Run:
