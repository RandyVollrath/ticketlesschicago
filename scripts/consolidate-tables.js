...
async function consolidateTables() {
  console.log('🔄 Starting table consolidation: users → user_profiles');
  ...
  try {
    ...
    for (const user of users || []) {
      try {
        processed++;
        console.log(`\n${processed}. Processing user: ${user.email}`);
    ...
    console.error('❌ Fatal error during consolidation:', error.message);
  }
  ...
  if (errors === 0) {
    console.log('\n🎉 Table consolidation completed successfully!');
    console.log('✅ All user data is now in the user_profiles table');
  } else {
    console.log('\n⚠️ Consolidation completed with some errors. Review logs above.');
  }
}

// Export for use as a module or run directly
if (require.main === module) {
  consolidateTables().catch(console.error);
}

module.exports = { consolidateTables };