...
export async function scrapeRedLightEvidence(
  ...
  try {
    ...
    for (const url of out.imageUrls) {
      ...
      if (fetched && isViolationImageByBytes(fetched.bytes, fetched.contentType)) {
 ...
 */
function isViolationImageByBytes(bytes: Buffer, contentType: string): boolean {
  if (!contentType.startsWith('image/')) return false;
  if (/svg|gif/i.test(contentType)) return false;
  // Anything under 20KB is almost certainly a UI element, not a 4K Canon
  // photo. Smallest real Chicago violation thumb we saw in testing was
  // 170KB; chrome assets were <2.5KB. 20KB is a safe floor.
