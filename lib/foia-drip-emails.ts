...
export async function sendFoiaDripDay7(params: {
  ...
  const html = quickEmail({
    ...
    body: [
      ...
      section("What's included in the Plan for $79/year", bulletList([
      ...
      hasRecords
        ? p(`Your FOIA showed you've paid <strong>$${(params.totalFines || 0).toLocaleString()}</strong> in Chicago tickets. The Chicago Ticket Defense Plan is $79/year. Even one dismissed ticket pays for the year.`)
        : p('$79/year. Less than two parking tickets. One dismissal pays for the year.', { center: true, size: '14px' }),
      button('Start the Chicago Ticket Defense Plan — $79/year', 'https://autopilotamerica.com/get-started', { color: '#10B981' }),
