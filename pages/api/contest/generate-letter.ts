...
import { runAllUICChecks, type ErroneousFinding } from '../../../lib/contest-review/uic-erroneous-checks';
...
7. If vehicle info is provided above, reference the specific vehicle (make, model, plate) to tie the evidence to the ticketed vehicle`;
    ...
    // 10. UIC-style Erroneous-Issuance Checks
    // Replicates the same retrospective verification IRRPP used to identify
    // 475,106 erroneous tickets in their 2022 study — applied to this single
    // ticket. Runs per-category checks (street cleaning time, special-events
    // permits, winter ban triple check, 2-inch snow, Loop polygon, CBD
    // bounds, address transposition) and produces hard findings the letter
    // can use as primary defenses.
    if (contest.ticket_location && ticketDate) {
      evidencePromises.push((async () => {
        try {
          // Resolve lat/lng for the cited address. Prefer pre-computed
          // coordinates from the ticket row; geocode otherwise.
          let lat: number | null = contest.ticket_latitude ?? null;
          let lng: number | null = contest.ticket_longitude ?? null;
          if (lat == null || lng == null) {
            try {
              const geo = await geocodeChicagoAddress(contest.ticket_location);
              if (geo.status === 'OK' && typeof geo.lat === 'number' && typeof geo.lng === 'number') {
                lat = geo.lat;
                lng = geo.lng;
              }
          ...
          // Build the ISO datetime in Chicago local time when we have a
          // time on the citation. The string is interpreted as local time
          // by the verifier — no timezone math needed.
          const ticketTime = contest.extracted_data?.time || null;
          const issueDateTime = ticketTime
            ? `${ticketDate}T${normalizeTimeToHHMMSS(ticketTime)}`
            : null;
          uicFindings = await runAllUICChecks(
            {
          ...
          if (uicFindings.length > 0) {
            console.log(`  UIC erroneous-issuance checks fired: ${uicFindings.map(f => f.id).join(', ')}`);
            uicFindingsText = `
=== UIC-STYLE ERRONEOUS-ISSUANCE FINDINGS (LEAD WITH ANY THAT APPLY) ===

The following findings were generated by replicating the methodology used by the University of Illinois Chicago's Institute for Research on Race and Public Policy in their 2022 study "475,106 Mistakes: When Tickets Are Issued Under False Pretenses." Each finding represents a verifiable mismatch between the ticket and the City's own data showing the ordinance the ticket cites was not in effect at the time and place of citation.

${uicFindings.map((f, i) => `[${i + 1}] ${f.title}
ORDINANCE: ${f.ordinance}
DEFENSE PARAGRAPH (incorporate verbatim or near-verbatim into the letter):
${f.defenseParagraph}
`).join('\n')}

INSTRUCTIONS:
- These are the STRONGEST possible defenses for this ticket — they go directly to the ordinance's prima facie case.
...
`;
    ...
    // User self-reported their plate was stolen via the contest form. Two
    // paths:
    //  (a) A matching detected_tickets row exists (autopilot subscribers).
    //      Persist there so downstream FOIA/audit logic sees the facts, and
    //      mirror onto detectedTicketData so the defense block below picks
    //      them up this run. We never clobber higher-confidence fields that
    //      were already set by a forwarded-police-report extraction.
    //  (b) No detected_tickets row exists (one-shot contest uploads). Build
    //      an in-memory detectedTicketData stub so the stolen-plate defense
    ...
    if (plateStolenInput === true) {
      ...
      if (detectedTicketData?.id) {
        ...
        // No detected_tickets row — build an in-memory stub so the defense
        // still applies. We're not persisting because there's no row keyed
        // to this contest; the user's attestation lives only on this letter.
        detectedTicketData = {
          ...(detectedTicketData || {}),
          plate_stolen: true,
    ...
    } | null = null;
    {
      ...
      // violationType is not always populated by the user-upload path (only
      // the autopilot scraper sets it). Fall back to violation_code and a
      // description regex so camera/missing-plate tickets uploaded via the
      // UI still qualify for the defense.
      const vCode = contest.violation_code || '';
      const vDesc = (contest.violation_description || '').toLowerCase();
      const isCameraOrPlate =
        ...
        vCode === '9-101-020' ||
        vCode === '9-80-040' ||
        /red\s*light|speed\s*camera|automated\s+(traffic|enforcement|red|speed)|missing.*plate|no.*plate.*display|plate.*not.*display/.test(vDesc);
...
4. The absence of supporting documentation raises questions about the reliability and completeness of the enforcement record, and any ambiguity from those missing records should be construed against the City
5. Frame as a transparency and procedural-fairness concern that strengthens the other substantive arguments — not as independently dispositive.`);
...
INSTRUCTIONS: This is a STRONG argument for red light camera tickets. Do NOT use the phrase "due process violation" — it is too strong as a blanket argument. Use this procedural-fairness framing alongside the substantive 625 ILCS 5/11-306(c-5) point: "Respondent timely requested records relevant to preparing a defense. The City failed to timely produce them, which impaired respondent's ability to evaluate the evidence and present a full defense. Any ambiguity created by the missing records should be construed against the City, and the citation should be dismissed." Without the signal timing plan, there is no way to verify that the yellow change interval complied with Illinois law (625 ILCS 5/11-306(c-5)) — Chicago has been caught violating this before (2014 Inspector General investigation). Because the City controls these records and failed to produce them, any ambiguity about statutory compliance should be construed against the City.`);
...
/**
 * Normalize a free-form time-of-day string to HH:MM:SS for the UIC checker.
 * Accepts "14:30", "2:30 PM", "02:30:00 pm", etc. Falls back to "00:00:00"
 * when unparseable — the verifier then skips time-of-day checks instead of
 * firing on noise.
 */
function normalizeTimeToHHMMSS(raw: string): string {
  ...
  if (m24) {
    ...
    const mi = Math.min(59, parseInt(m24[2], 10));
    const se = m24[3] ? Math.min(59, parseInt(m24[3], 10)) : 0;
    return `${String(h).padStart(2, '0')}:${String(mi).padStart(2, '0')}:${String(se).padStart(2, '0')}`;
  }
  const m12 = s.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)$/);
  if (m12) {
    let h = parseInt(m12[1], 10);
    const mi = Math.min(59, parseInt(m12[2], 10));
    const se = m12[3] ? Math.min(59, parseInt(m12[3], 10)) : 0;
    if (m12[4] === 'PM' && h < 12) h += 12;
    if (m12[4] === 'AM' && h === 12) h = 0;
    return `${String(h).padStart(2, '0')}:${String(mi).padStart(2, '0')}:${String(se).padStart(2, '0')}`;
  }
  return '00:00:00';
}
