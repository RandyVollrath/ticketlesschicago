...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Send new order notification email to remitter/partner
    await sendNewOrderEmailToPartner(order, partner);

    // Webhook to partner (if they have a webhook configured)
...
/**
 * Send email notification to remitter/partner when they receive a new order
 */
async function sendNewOrderEmailToPartner(order: any, partner: any) {
  if (!process.env.RESEND_API_KEY) {
    console.warn('RESEND_API_KEY not set, skipping partner notification email');
    return;
  }

  const partnerEmail = partner.email;
  if (!partnerEmail) {
    console.warn('Partner has no email address, skipping notification');
    return;
  }

  const portalUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://autopilotamerica.com'}/remitter-portal`;
  const formattedAmount = typeof order.total_amount === 'number'
  ...
  try {
    const response = await fetchWithTimeout('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Autopilot America <alerts@autopilotamerica.com>',
        to: partnerEmail,
        subject: `🆕 New Order Received - ${order.license_plate} (#${order.order_number})`,
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 24px; background: #f9fafb; border-radius: 0 0 8px 8px;">
              <p style="color: #374151; font-size: 16px;">Hi ${partner.name},</p>

              <p style="color: #374151; font-size: 16px;">
                You have a new city sticker renewal order ready for processing.
              </p>

              <div style="background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin: 24px 0;">
                <h3 style="margin: 0 0 12px 0; color: #1f2937; font-size: 14px; text-transform: uppercase;">Order Details</h3>
                <table style="width: 100%; border-collapse: collapse;">
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280;">Customer</td>
                    <td style="padding: 8px 0; color: #1f2937; text-align: right; font-weight: 600;">${order.customer_name}</td>
                  </tr>
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280;">License Plate</td>
                    <td style="padding: 8px 0; color: #1f2937; text-align: right; font-weight: 600;">${order.license_plate}</td>
                  ...
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280;">Sticker Type</td>
                    <td style="padding: 8px 0; color: #1f2937; text-align: right; font-weight: 600;">${order.sticker_type || 'Standard'}</td>
                  </tr>
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280;">Address</td>
                    <td style="padding: 8px 0; color: #1f2937; text-align: right; font-weight: 600;">${order.street_address}, ${order.city}</td>
                  </tr>
                  <tr style="border-top: 1px solid #e5e7eb;">
                    <td style="padding: 12px 0 0 0; color: #6b7280; font-weight: 600;">Total Amount</td>
                    <td style="padding: 12px 0 0 0; color: #1f2937; text-align: right; font-weight: 700; font-size: 18px;">${formattedAmount}</td>
                  </tr>
                </table>
              </div>
              ...
              <div style="background: #ecfdf5; border-left: 4px solid #10b981; padding: 12px 16px; margin: 24px 0;">
                <p style="margin: 0; color: #065f46; font-size: 14px;">
                  <strong>Action Required:</strong> Log in to your portal to view full details and process this order.
                </p>
              </div>
              ...
              <p style="color: #9ca3af; font-size: 12px; margin: 0;">
                Autopilot America • Remitter Partner Program
              </p>
            </div>
          </div>
        `
    ...
    if (response.ok) {
      console.log(`✅ Sent new order notification email to partner ${maskEmail(partnerEmail)}`);
    } else {
      const error = await response.text();
