...
export default function AlertsSignup() {
  ...
  // Handle OAuth flow - pre-fill email from authenticated user
  useEffect(() => {
    const flow = router.query.flow as string;
    const email = router.query.email as string;

    if (flow === 'oauth' && email) {
      console.log('🔐 OAuth signup flow detected, pre-filling email:', email);
      setFormData(prev => ({
        ...prev,
        email: email
      }));
      ...
      <main style={{
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit} style={{
            ...
            <div>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: '16px',
                  boxSizing: 'border-box',
                  backgroundColor: router.query.flow === 'oauth' ? '#f9fafb' : 'white',
                  cursor: router.query.flow === 'oauth' ? 'not-allowed' : 'text'
                }}
              />
            </div>
            ...
            >
              {loading ? 'Creating Your Account...' : (router.query.flow === 'oauth' ? 'Complete Signup' : '📧 Get Free Alerts (Email Link)')}
            </button>

            {/* Only show Google button if NOT already OAuth authenticated */}
