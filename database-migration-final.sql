-- FINAL CLEAN DATABASE MIGRATION SCRIPT
-- This handles empty strings and null dates properly
...
-- Migrate city sticker obligations (with proper date validation)
...
WHERE vr.city_sticker_expiry IS NOT NULL
  ...
  AND vr.city_sticker_expiry ~ '^\d{4}-\d{2}-\d{2}$' -- Valid date format
ORDER BY v.id, vr.city_sticker_expiry::date, vr.created_at DESC;

-- Migrate license plate obligations (with proper date validation)
INSERT INTO obligations (vehicle_id, user_id, type, due_date, completed, created_at)
...
WHERE vr.license_plate_expiry IS NOT NULL
  ...
  AND vr.license_plate_expiry ~ '^\d{4}-\d{2}-\d{2}$' -- Valid date format
ORDER BY v.id, vr.license_plate_expiry::date, vr.created_at DESC;

-- Migrate emissions obligations (with proper date validation)
INSERT INTO obligations (vehicle_id, user_id, type, due_date, completed, created_at)
...
WHERE vr.emissions_due_date IS NOT NULL
  ...
  AND vr.emissions_due_date ~ '^\d{4}-\d{2}-\d{2}$' -- Valid date format
ORDER BY v.id, vr.emissions_due_date::date, vr.created_at DESC;
...
-- Test the reminder function for today (0 days ahead) - should show license plate due today
