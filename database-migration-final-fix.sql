-- FINAL WORKING DATABASE MIGRATION SCRIPT
...
-- Migrate city sticker obligations (only where date is not null)
INSERT INTO obligations (vehicle_id, user_id, type, due_date, completed, created_at)
SELECT DISTINCT ON (v.id, vr.city_sticker_expiry)
  v.id as vehicle_id,
  v.user_id,
...
ORDER BY v.id, vr.city_sticker_expiry, vr.created_at DESC;

-- Migrate license plate obligations (only where date is not null)
INSERT INTO obligations (vehicle_id, user_id, type, due_date, completed, created_at)
SELECT DISTINCT ON (v.id, vr.license_plate_expiry)
  v.id as vehicle_id,
  v.user_id,
  'license_plate' as type,
  vr.license_plate_expiry as due_date,
...
ORDER BY v.id, vr.license_plate_expiry, vr.created_at DESC;

-- Migrate emissions obligations (only where date is not null)
INSERT INTO obligations (vehicle_id, user_id, type, due_date, completed, created_at)
SELECT DISTINCT ON (v.id, vr.emissions_due_date)
  v.id as vehicle_id,
  v.user_id,
  'emissions' as type,
...
ORDER BY v.id, vr.emissions_due_date, vr.created_at DESC;
...
SELECT 
  ...
  u.email,
  u.phone,
  (CURRENT_DATE - o.due_date)::integer as days_overdue
FROM obligations o
JOIN vehicles v ON o.vehicle_id = v.id
JOIN users u ON o.user_id = u.id
WHERE o.completed = false
...
-- Test the reminder function for today (should find license plates due today: 2025-09-19)
SELECT 'Obligations due TODAY (should find license plates):' as info;
SELECT * FROM get_obligations_needing_reminders(0);

-- Test for tomorrow 
SELECT 'Obligations due TOMORROW:' as info;
